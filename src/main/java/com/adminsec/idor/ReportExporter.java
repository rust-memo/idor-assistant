...
public final class ReportExporter {
    public void export(Path path, List<Candidate> candidates) throws IOException {
        ...
        if (lower.endsWith(".json")) exportJson(path, candidates);
        else if (lower.endsWith(".html") || lower.endsWith(".htm")) exportHtml(path, candidates);
        else new CsvExporter().export(path, candidates);
    }

    private void exportJson(Path path, List<Candidate> candidates) throws IOException {
        JsonArray rows = new JsonArray();
        for (Candidate c : candidates) rows.add(asJson(c));
    ...
    private JsonObject asJson(Candidate c) {
        ...
        row.add("references", refs); return row;
    }

    private void exportHtml(Path path, List<Candidate> candidates) throws IOException {
        StringBuilder html = new StringBuilder("<!doctype html><meta charset=\"utf-8\"><title>IDOR/BOLA report</title>"
                ...
                + "<h1>IDOR/BOLA Assistant report</h1><p>Metadata-only export; authentication, query values and message bodies are excluded.</p>"
